...
const handleRecaptchaChange = (value) => {
  ...
  // Update the text content of captchatextRef based on Recaptcha value
  if (value) {
    captchatext.current.innerText = ''; // Clear the text
  } else {
    captchatext.current.innerText = 'Invalid captcha';
  }
};
...
const handleSubmit = (event) => {
  ...
  captchatext.current.innerText='Enter captcha'
  }
};
...
useEffect(() => {
  const captchaSize= () => {
    ...
    if (window.innerWidth < 425) {
      ...
      setcaptchasize(false)    
    }}

    captchaSize()
    
    const handleResize = () => {
      captchaSize() // Adjust scale on window resize
    };
          ...
          <div className="form-group col-10" style={{ paddingLeft: "25px", }}>
            ...
            <form onSubmit={handleSubmit} className="row g-3" action="" method="post" id="contact_form">
              <fieldset>
                ...
                <div style={{ transform: "scaleX(0.77) scaleY(0.85)",transformOrigin:"left"}} className=" captcha mb-3"> 
                ...
                { captchasize ? <> 
                    ...
                    <ReCAPTCHA
                      ref={recaptchaRef}
                      sitekey="6LcRJZspAAAAAEL6YjhhUzFhbUTzv88JLGlHMR1y"
                      onChange={handleRecaptchaChange } size="compact"
                        />
                        ...
                        ref={recaptchaRef}
                        sitekey="6LcRJZspAAAAAEL6YjhhUzFhbUTzv88JLGlHMR1y"
                        onChange={handleRecaptchaChange } size="normal"
                          />
                         </> 
                         
                }
                   <div ref={captchatext} className="captchatext"></div>
                  </div>
